...
input_country = st.text_input("Enter a country:", "").strip()
...
if input_country:
    if input_country in country_data['country'].tolist():
        country_data.drop(country_data[country_data['country'] == input_country].index, inplace=True)
        last_letter = input_country[-1].lower()
        if last_letter in letter_bank:
            letter_bank[last_letter] -= 1
            if letter_bank[last_letter] == 0:
